...
echo "  ██║ ╚═╝ ██║██║  ██║███████╗███████╗"
echo "  ╚═╝     ╚═╝╚═╝  ╚═╝╚══════╝╚══════╝"
echo -e "${RESET}${BOLD}  Network Security Monitor  —  Installer${RESET}"
blank
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "  --uninstall   Remove a previously installed Maze [requires root for system install]"
      exit 0
      ;;
  esac
done
...
if $USER_MODE; then
  ...
  SUDOERS_FILE=""   # cannot create sudoers entry without root
  NEED_ROOT=false
else
  ...
  SUDOERS_FILE="/etc/sudoers.d/maze"
  NEED_ROOT=true
fi
...
# ── Uninstall path ────────────────────────────────────────────────────────────
do_uninstall() {
  step "Uninstalling Maze"
  local dirs=("$INSTALL_DIR")
  local files=("$LAUNCHER" "$DESKTOP_DIR/maze.desktop")
  [[ -n "$SUDOERS_FILE" ]] && files+=("$SUDOERS_FILE")
...
#!/usr/bin/env bash
# Maze launcher — generated by install.sh
...
# ── Optional sudoers entry ────────────────────────────────────────────────────
setup_sudoers() {
  [[ -z "$SUDOERS_FILE" ]] && return   # user-mode: skip

  local HELPER_ABS="$INSTALL_DIR/maze/helper.py"
  local VENV_PY_ABS
  VENV_PY_ABS=$(readlink -f "$INSTALL_DIR/venv/bin/python3" 2>/dev/null \
    || echo "$INSTALL_DIR/venv/bin/python3")

  blank
  echo -e "${YELLOW}${BOLD}Optional: skip sudo password prompt${RESET}"
  echo "  Maze asks for your sudo password each time to start its privileged helper."
  echo "  A targeted sudoers entry can remove that prompt for this specific binary:"
  blank
  echo -e "  ${BOLD}$CURRENT_USER ALL=(root) NOPASSWD: $VENV_PY_ABS $HELPER_ABS *${RESET}"
  blank
  echo "  This is limited to the exact helper path — no other commands are affected."
  blank

  read -r -p "  Create this sudoers entry? [y/N] " ans
  if [[ "${ans,,}" == "y" ]]; then
    printf '%s ALL=(root) NOPASSWD: %s %s *\n' \
      "$CURRENT_USER" "$VENV_PY_ABS" "$HELPER_ABS" \
      > "$SUDOERS_FILE"
    ...
    if visudo -c -f "$SUDOERS_FILE" &>/dev/null; then
      ...
      rm -f "$SUDOERS_FILE"
      warn "Sudoers validation failed — entry removed. Password prompt will appear at startup."
    fi
  else
    info "Skipped — password will be requested at each startup."
  fi
}
...
#!/usr/bin/env bash
# Maze uninstaller — generated by install.sh
set -euo pipefail
...
main() {
  ...
  $NEED_ROOT && setup_sudoers

  blank
  echo -e "${GREEN}${BOLD}  ✓  Maze installed successfully!${RESET}"
  blank
  echo -e "  Launch:    ${BOLD}maze${RESET}"
  echo -e "             or find Maze in your application launcher"
